...
export const TreeOfVerse: React.FC<TreeOfVerseProps> = ({ rotation, onVerseSelect, onBulkExport }) => {
    ...
    const renderDiagram = () => (
        <div className="flex justify-center w-full h-full p-2">
            <svg viewBox="0 0 150 280" width="100%" height="100%" aria-hidden="true" className="drop-shadow-[0_0_15px_rgba(6,182,212,0.4)]">
                <style>{`
                ...
                <g strokeWidth="1.2">
                    {KATHARA_GRID_LINES.filter(l => l.from <= 12 && l.to <= 12).map((line, index) => {
                        ...
                        return <line key={index} x1={fromNode.x} y1={fromNode.y} x2={toNode.x} y2={toNode.y} stroke={index % 2 === 0 ? "#06b6d4" : "#d946ef"} className="tree-line" />;
                    })}
                </g>
                {KATHARA_GRID_NODES.slice(0, 12).map((node, index) => {
                    ...
                    return (
                        <g key={node.id}>
                            <circle cx={node.x} cy={node.y} r={10} fill="#000" stroke="#06b6d4" strokeWidth="1.5" />
                            <text x={node.x} y={node.y} textAnchor="middle" dy=".3em" fontSize="7" fontWeight="bold" fill="#fff" style={{ textShadow: '0 0 4px #06b6d4' }}>
                                {firstVerse.surah}:{firstVerse.ayah}
                            </text>
                            <text x={node.x} y={node.y - 12} textAnchor="middle" fontSize="5" fill="#94a3b8" fontWeight="bold" className="uppercase">
                                {TREE_LABELS[index]}
                            </text>
                        </g>
                    );
    ...
    return (
        <div className="pt-4 animate-in fade-in slide-in-from-right duration-500">
            ...
            {!isDesktop ? (
                <div className="my-4 h-[350px] bg-black/20 rounded-xl border border-white/5">{renderDiagram()}</div>
            ) : (
